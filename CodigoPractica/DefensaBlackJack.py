...
import externoLunes as externo
import os
...
class Jugador(Mano):
    ...
    def imprime_jugador(self):
        ...
        print("Longitud manos: ", len(self.nombre_mano))
        for i in range(len(self.manos)):
            if i > 0:
                print(" │ ", end='\0')
    ...
    def separarMano2(self, cartas_iguales, indice_mano):
        nombres = []
        for i in range(4):
            nombre = self.nombre_mano[indice_mano]
            nueva_letra = chr(ord(nombre[:-1]) + 1)
            nombreFinal = nombre[:-1] + nueva_letra
...
def dime_cartas(jugador, i):
    ...
    if len(jugador.manos[i].cartas) > 1:
        for j in range(len(jugador.manos[i].cartas) - 1): # Recorro las cartas (carta que comparo)
            ...
            # Compruebo si las cartas son iguales o no (no pueden valer lo mismo j y q (es la misma carta))
            if j != q and carta == otra_carta:
                cartas_iguales.append(jugador.manos[i].cartas[q])
...
#### DEFENSA ####
#################
def dime_cartas_repetidas(jugador, i):
    # Variable para ver si hay cartas iguales
    cartas_iguales = 1
    # Si hay mas de una carta en la mano, comprobamos si hay 2 cartas iguales
    if len(jugador.manos[i].cartas) > 1:
        for j in range(len(jugador.manos[i].cartas) - 1): # Recorro las cartas (carta que comparo)
            ...
            if j != q and carta == otra_carta:
                cartas_iguales += 1
                # Si carta == otra_carta, sumamos 1 a la variable ya que tienen el mismo valor de carta
    print("Cartas iguales: ", cartas_iguales)
    return cartas_iguales
...
### MODO JUEGO ####
###################
def modoJuego(mazo, balance, contador_partidas, mazo2):
    ...
    while partida:
        ...
        croupier.mano.agregar_carta(mazo2.reparte())
        
        for _ in range(2):
            comprueba_genera_mazo(mazo)
            jugador.agregar_carta_jugador(0, mazo2.reparte())        # El 0 hace referencia a la mano inicial del jugador
                ...
                for i in range(len(jugador.manos)):     # Recorro todas las manos del jugador
                    if jugador.estado_mano[i] in ["Cerrada", "PASADA"]:     # En este caso, el jugador no podra gestionar la mano en cuestion y lo mostramos por pantalla
                        ...
                        if jugada in ["P", "p"]:        # Pedimos y agregamos una carta a la mano en cuestion
                            comprueba_genera_mazo(mazo)
                            jugador.agregar_carta_jugador(i, mazo2.reparte())        # i hace referencia a la mano, mazo.pop() inserta una carta del mazo
                            ...
                            comprueba_genera_mazo(mazo)
                            jugador.agregar_carta_jugador(i, mazo2.reparte())        # Agregamos una carta a la mano correspondiente
                            ...
                            jugador.separarMano2(dime_cartas(jugador, i), i)     # Separamos la mano cuando haya 2 cartas con el mismo valor (Ej: 7 y 7)
...
def modoPredeterminado(mazo, balance, contador_partidas, mazo2):
    croupier = Croupier()       # Creo el croupier
    jugador = Jugador()     # Creo al jugador
    jugador.agregar_mano()      # Le doy una mano al jugador
    ...
    contador_partidas += 1
    modoJuego(mazo, balance, contador_partidas, mazo2)
...
def Main():
    ...
    mazo2 = externo.Mazo(MiCarta,estrategia)
    ...
    while bucleCorrecto:
        ...
        if modoEjecucion in ["J", "j"]:
            ...
            modoPredeterminado(mazo, balance, contador_partidas, mazo2)
            bucleCorrecto = False
